...
export function getSumOfMiddleElementsInCorrectlyOrderedPages(pages: number[][], orderRules: string) {
  const parsedOrderRules = parseOrderRules(orderRules);

  return pages
    .filter((page) => isPagePrintedInCorrectOrder(page, parsedOrderRules))
    .reduce((acc, page) => acc + page[Math.floor(page.length / 2)], 0);
}
